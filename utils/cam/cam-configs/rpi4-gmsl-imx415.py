...
imx415_h = 1080
if USE_RAW_10:
    imx415_bus_fmt = v4l2.BusFormat.SRGGB10_1X10
    imx415_pix_fmt = v4l2.PixelFormats.SRGGB10P
else:
...
def gen_imx415_meta(des_ent, des_src_pad, ch_index, cameras, port):
    ...
    return {
        ...
        'links': [
            ...
            { 'src': (ser_ent, 1), 'dst': (des_ent, port) },
            { 'src': (des_ent, des_src_pad), 'dst': (CSI2_NAME, 0) },
            { 'src': (CSI2_NAME, 1 + ch_index), 'dst': ('unicam-embedded', 0) },
        ],
    }
...
def get_configs(config_names):
    ...
    for i in cameras:
        ...
        if cam in config_names:
            ...
            cfg = gen_imx415_pixel(des_name, des_src_pad, ch_index, cameras, i)
            cfgs.append(cfg)
            ch_index += 1

        if cam_meta in config_names:
            config_names.remove(cam_meta)
            cfg = gen_imx415_meta(des_name, des_src_pad, ch_index, cameras, i)
            cfgs.append(cfg)
            ch_index += 1
